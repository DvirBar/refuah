...
import { UserDoc, UserSecureDetails } from "./db/types";
...
export const verifyPasswordToken = (
	token: string,
): string | jwt.JwtPayload => jwt.verify(token, process.env.JWT_SECRET_RESET as jwt.Secret);
...
export const createPasswordResetToken = (payload: Payload): string => jwt.sign(
	payload,
        process.env.JWT_SECRET_RESET as jwt.Secret,
...
export const userWithoutPassword = (user: UserDoc): UserSecureDetails => {
	const {
		password,
		formerPasswords,
		failedAttempts,
